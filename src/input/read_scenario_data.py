...
def read_scenario_data(dirpath: str, scenarios_actual: dict, scenarios_bidding: dict, projects: pd.DataFrame): 
    ...
    data_actual.prices = years_to_rows(data_actual.prices, year_name = "Period", value_name = "Price")
    data_actual.free_allocations = years_to_rows(data_actual.free_allocations, year_name = "Period", value_name = "Free Allocations")
    
    data_bidding.prices = years_to_rows(data_bidding.prices, year_name = "Period", value_name = "Price")
    data_bidding.free_allocations = years_to_rows(data_bidding.free_allocations, year_name = "Period", value_name = "Free Allocations")
...
def years_to_rows(data: pd.DataFrame, year_name: str, value_name: str):
    years = [x for x in data.columns if x.isdigit()]
    id_vars = [x for x in data.columns if not x.isdigit()]
    data = data.melt(
        id_vars=id_vars,
        value_vars=years,
        var_name=year_name, 
