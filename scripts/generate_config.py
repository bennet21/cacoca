        ...
        if isinstance(value, dict):
            ...
            for key_, value_ in value.items():
                ...
                if value_:
                    ...
                    selected_scenario \
                        = read_all_component_scenarios(key_, scenario_dir)["Scenario"].iloc[0]
...
def read_all_component_scenarios(component: str, scenario_dir: str):
    all_prices, all_free_allocations, h2share = read_raw_scenario_data(dirpath=scenario_dir)

    if component.startswith('Prices '):
        return all_prices.query(f"Component=='{component.replace('Prices ', '')}'") \
            .filter(items=['Scenario']) \
            .drop_duplicates()
    else:
        return all_free_allocations.filter(items=['Scenario']).drop_duplicates()
...
def read_all_by_columns(scenario_dir: str, groupByColumn: str, valueColumn: str):
    ...
    prices = pd.concat([
        all_prices.query(f"{groupByColumn}=='{component.replace('Prices ', '')}'")
        .filter(items=[groupByColumn, valueColumn])
        .drop_duplicates()
        for component in all_components if component.startswith("Prices ")
    ])
    free_allocations = all_free_allocations.filter(items=[valueColumn]).drop_duplicates()
...
def select_scenario(component: str, scenario: str, scenario_dir: str):
    ...
    if scenario in all_scenarios \
            .query(f"Component=='{component.replace('Prices ', '')}'")[["Scenario"]].values:
